...
const allPushed =
  git_status.search("Your branch is up to date with 'origin/main'.") >= 0;
...
if (!allPushed) {
  console.log(
    "Error: There are commits that are not pushed to the server yet, will not perform release!"
  );
}
...
if (answer === "y") {
  ...
  console.log(
    "Cancelled, version remains the same and no new release is built!"
  );
}
