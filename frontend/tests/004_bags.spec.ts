...
test("2. add bag and edit", async ({ page }) => {
  ...
  await expect(
    page.getByRole("application", { name: "Event Dec" })
  ).not.toBeVisible();
  await expect(page.locator('[data-test-id="btn-Roast Date"]')).toContainText(
    "12/15/2025"
  );
  ...
  await expect(
    page.locator('[data-test-id="btn-Purchase Date"]')
  ).toContainText("12/16/2025");
  ...
  await page
    .getByRole("spinbutton", { name: "Initial Weight (g) *" })
    .fill("500");
...
test("3. add bag 2", async ({ page }) => {
  ...
  await expect(page.locator('[data-test-id="btn-Opened Date"]')).toContainText(
    "12/17/2025"
  );
...
test("4. add bean 3", async ({ page }) => {
  ...
  await expect(
    page.locator('[data-test-id="btn-Finished Date"]')
  ).toContainText("12/29/2025");
...
test("5. list view", async ({ page }) => {
  await page.goto("./");
  await page.getByRole("button", { name: "Bags" }).click();
  await expect(page.locator('[data-test-id="tg_bagfilter"]'))
    .toMatchAriaSnapshot(`
    - group:
      - radio "Unopened"
