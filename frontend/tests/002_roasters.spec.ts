...
test("2. add roaster and edit", async ({ page }) => {
  ...
  await expect(
    page.getByRole("link", { name: "https://www.roaster.com" })
  ).toBeVisible();
...
test("3. add roaster 2", async ({ page }) => {
  ...
  await page
    .getByRole("textbox", { name: "Website URL (Optional)" })
    .fill("https://www.roaster2.com");
  // await page.getByRole('button', { name: 'Roaster Image (Optional)' }).setInputFiles('logo.svg');
  await page.getByRole("button", { name: "Create Roaster" }).click();
  await expect(page.getByText("Roaster 2")).toBeVisible();
  await expect(
    page.getByRole("link", { name: "https://www.roaster2.com" })
  ).toBeVisible();
  await expect(page.getByText("Roaster Details")).toBeVisible();
});
...
test("4. add roaster 3", async ({ page }) => {
  ...
  await page
    .getByRole("textbox", { name: "Website URL (Optional)" })
    .fill("https://www.roaster3.com");
  // await page.getByRole('button', { name: 'Roaster Image (Optional)' }).setInputFiles('logo.svg');
  await page.getByRole("button", { name: "Create Roaster" }).click();
  await expect(page.getByText("Roaster 3")).toBeVisible();
  await expect(
    page.getByRole("link", { name: "https://www.roaster3.com" })
  ).toBeVisible();
  await expect(page.getByText("Roaster Details")).toBeVisible();
});
...
test("5. list view", async ({ page }) => {
  ...
  await expect(
    page
      .locator('[data-test-id="card-Roaster 1"]')
      .getByRole("link", { name: "https://www.roaster1.com" })
  ).toBeVisible();
  await expect(page.locator('[data-test-id="card-Roaster 2"]')).toBeVisible();
  await expect(
    page
      .locator('[data-test-id="card-Roaster 2"]')
      .getByRole("link", { name: "https://www.roaster2.com" })
  ).toBeVisible();
  await expect(page.locator('[data-test-id="card-Roaster 3"]')).toBeVisible();
  await expect(
    page
      .locator('[data-test-id="card-Roaster 3"]')
      .getByRole("link", { name: "https://www.roaster3.com" })
  ).toBeVisible();
});
